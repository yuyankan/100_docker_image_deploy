...
HOST_PROJECTS_PATH = '/home/carenk/my_data/01_projects'

with DAG(
    dag_id='etl_spc_c11_spec_control',
    # Ensure start_date is timezone-aware
    start_date=datetime(2025, 12, 16, 0, 0, tzinfo=beijing_tz),
    schedule_interval='0 0 * * 0', #weekly
    catchup=False, # Important: Set to False for new DAGs to avoid backfilling old runs
    tags=['etl', 'spc', 'c11','spec_control'],
) as dag:
    run_etl_script = DockerOperator(
        task_id='execute_etl_spc_spec_control',
        ...
        command='bash -c "cd /app/projects/01_project_spc/01_etl_main/02_etl_scaled && python spc_etl_control_spec_calculate.py"',
