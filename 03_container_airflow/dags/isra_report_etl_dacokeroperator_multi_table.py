...
HOST_PROJECTS_PATH = '/home/carenk/my_data/01_projects/04_isra/01_etl_result/02_etal_multi_thread_task_table'

DATA_MIRROR_PATH = '/home/carenk/my_data/01_projects/04_isra/02_fastapi/mnt/images'

with DAG(
    dag_id='etl_isra_report_etl_main_multi_thread_task_table',
    # Ensure start_date is timezone-aware
    max_active_runs=1,  # 确保同一时间只有一个 DAG 实例在运行
    start_date=datetime(2025, 9, 1, 0, 0, tzinfo=beijing_tz),
    schedule_interval='*/30 * * * *', # every 10 minutes
    ...
    run_etl_script = DockerOperator(
        ...
        # keep that tag.
        image='python_env_common:202507', # Assuming you built the new image with Playwright
        # Command to run inside the newly spawned container
        command='bash -c "cd /app/projects/ && python etl_main_scale_table.py"',
        ...
        mounts=[
            ...
            # 2. 新增的文件地址挂载（根据你的要求）
            # Mount the specified image directory from the host into the container.
            Mount(
                source=DATA_MIRROR_PATH,  # 宿主机路径
                target="/data_mount",                                                   # 容器内路径
                type='bind'                                                             # 绑定挂载类型
                ),
